use crate::scc::algo_async::{
    BwdProcess, DiscardNeverFiresAgain, GraphScheduler, Process, ReachAfterVarFired,
};
...
impl DiscardNeverFiresAgain {
    ...
    pub fn mk(
        variable: VariableId,
        never_fires_again: GraphColoredVertices,
...
impl Process for DiscardNeverFiresAgain {
    fn step(&mut self, scheduler: &mut GraphScheduler, graph: &SymbolicAsyncGraph) -> bool {
        if self.bwd.step(scheduler, graph) {
            ...
            scheduler.spawn(Box::new(ReachAfterVarFired::mk(
                self.variable,
                scheduler,
                graph,
            )));
            for v in graph.network().variables() {
                if !scheduler.has(&DiscardNeverFiresAgain::name_tmp(v))
                    && !scheduler.has(&ReachAfterVarFired::name_tmp(v))
                {
