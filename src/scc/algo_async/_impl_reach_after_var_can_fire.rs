use crate::scc::algo_async::{
    FindExtendedComponent, FwdProcess, GraphScheduler, Process, ReachAfterVarCanFire,
};
...
impl Process for ReachAfterVarCanFire {
    fn step(&mut self, scheduler: &mut GraphScheduler, graph: &SymbolicAsyncGraph) -> bool {
        if self.fwd.step(scheduler, graph) {
            ...
            println!(
                "Done reach after var can fire! {}",
                reach_after_var_can_fire.approx_cardinality()
            );
            scheduler.spawn(Box::new(FindExtendedComponent::mk(
                var_can_fire,
                reach_after_var_can_fire.clone(),
                scheduler,
            )));
            true
        } else {
