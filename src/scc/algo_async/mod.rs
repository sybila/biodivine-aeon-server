...
use std::io::Write;
use crate::scc::algo_effectively_constant::{reach_saturated_fwd_excluding, reach_saturated_bwd_excluding};
...
fn _check_useless(graph: &SymbolicAsyncGraph, universe: &GraphColoredVertices, condition: &GraphColoredVertices) -> bool {
    ...
    let t2 = below_condition;
    let t3 = condition_component.minus(&below_condition_basin);
    (t1.is_empty() && t2.is_empty()) | (t1.is_empty() && t3.is_empty()) | (t2.is_empty() && t3.is_empty())
}
