...
use std::str::Chars;
use crate::all::{BooleanFormula, StateAtom, ALLFormula, StateFormula, AttractorFormula, AttractorAtom, ALLAtom};
use crate::scc::Behaviour;


#[derive(Clone, Eq, PartialEq, Debug)]
enum BooleanExpressionToken<A> where A: Eq {
    Not, BinaryOp(BinaryOp), Tokens(Vec<BooleanExpressionToken<A>>), Atom(A)
}

type StateFormulaToken = BooleanExpressionToken<String>;

fn tokenize_boolean_expression<A, F>(data: &mut Peekable<Chars>, top_level: bool, try_tokenize_atom: &F) -> Result<Vec<BooleanExpressionToken<A>>, String>
    where F: Fn(&mut Peekable<Chars>, char) -> Option<A>, A: Eq {
...
fn parse_boolean_expression<AT, AF, F>(data: &[BooleanExpressionToken<AT>], parse_atom: &F) -> Result<Box<BooleanFormula<AF>>, String>
    where F: Fn(&AT) -> Result<AF, String>, AT: Eq {
    return iff(data, parse_atom)
}

/// **(internal)** Utility method to find first occurrence of a specific token in the token tree.
fn index_of_first<A>(data: &[BooleanExpressionToken<A>], token: BooleanExpressionToken<A>) -> Option<usize> where A: Eq {
    return data.iter().position(|t| *t == token);
}

/// **(internal)** Recursive parsing step 1: extract `<=>` operators.
fn iff<AT, AF, F>(data: &[BooleanExpressionToken<AT>], parse_atom: &F) -> Result<Box<BooleanFormula<AF>>, String>
    where F: Fn(&AT) -> Result<AF, String>, AT: Eq {
    let iff_token = index_of_first(data, BooleanExpressionToken::BinaryOp(BinaryOp::Iff));
    return Ok(if let Some(i) = iff_token {
        Box::new(BooleanFormula::Binary { op: BinaryOp::Iff, left: imp(&data[..i], parse_atom)?, right: iff(&data[(i + 1)..], parse_atom)? })
    } else {
        imp(data, parse_atom)?
    });
}
...
fn imp<AT, AF, F>(data: &[BooleanExpressionToken<AT>], parse_atom: &F) -> Result<Box<BooleanFormula<AF>>, String>
    where F: Fn(&AT) -> Result<AF, String>, AT: Eq {
    let imp_token = index_of_first(data, BooleanExpressionToken::BinaryOp(BinaryOp::Imp));
    return Ok(if let Some(i) = imp_token {
        Box::new(BooleanFormula::Binary { op: BinaryOp::Imp, left: imp(&data[..i], parse_atom)?, right: iff(&data[(i + 1)..], parse_atom)? })
    } else {
        or(data, parse_atom)?
    });
}
...
fn or<AT, AF, F>(data: &[BooleanExpressionToken<AT>], parse_atom: &F) -> Result<Box<BooleanFormula<AF>>, String>
    where F: Fn(&AT) -> Result<AF, String>, AT: Eq {
    let or_token = index_of_first(data, BooleanExpressionToken::BinaryOp(BinaryOp::Or));
    return Ok(if let Some(i) = or_token {
        Box::new(BooleanFormula::Binary { op: BinaryOp::Or, left: imp(&data[..i], parse_atom)?, right: iff(&data[(i + 1)..], parse_atom)? })
    } else {
        and(data, parse_atom)?
    });
}
...
fn and<AT, AF, F>(data: &[BooleanExpressionToken<AT>], parse_atom: &F) -> Result<Box<BooleanFormula<AF>>, String>
    where F: Fn(&AT) -> Result<AF, String>, AT: Eq {
    let and_token = index_of_first(data, BooleanExpressionToken::BinaryOp(BinaryOp::And));
    return Ok(if let Some(i) = and_token {
        Box::new(BooleanFormula::Binary { op: BinaryOp::And, left: imp(&data[..i], parse_atom)?, right: iff(&data[(i + 1)..], parse_atom)? })
    } else {
        xor(data, parse_atom)?
    });
}
...
fn xor<AT, AF, F>(data: &[BooleanExpressionToken<AT>], parse_atom: &F) -> Result<Box<BooleanFormula<AF>>, String>
    where F: Fn(&AT) -> Result<AF, String>, AT: Eq {
    let xor_token = index_of_first(data, BooleanExpressionToken::BinaryOp(BinaryOp::Xor));
    return Ok(if let Some(i) = xor_token {
        Box::new(BooleanFormula::Binary { op: BinaryOp::Xor, left: imp(&data[..i], parse_atom)?, right: iff(&data[(i + 1)..], parse_atom)? })
    } else {
...
fn terminal<AT, AF, F>(data: &[BooleanExpressionToken<AT>], parse_atom: &F) -> Result<Box<BooleanFormula<AF>>, String>
    where F: Fn(&AT) -> Result<AF, String>, AT: Eq {
    return if data.is_empty() {
        Err("Expected formula, found nothing.".to_string())
    } else if data[0] == BooleanExpressionToken::Not {
        Ok(Box::new(BooleanFormula::Not(terminal(&data[1..], parse_atom)?)))
    } else if data.len() != 1 {
        Err("Too many atom tokens in a formula.".to_string())
    } else {
        match &data[0] {
            BooleanExpressionToken::Atom(atom) => Ok(Box::new(BooleanFormula::Atom(parse_atom(atom)?))),
            BooleanExpressionToken::Tokens(inner) => Ok(parse_boolean_expression(inner, parse_atom)?),
...
pub fn parse_filter(bn: &BooleanNetwork, value: &str) -> Result<ALLFormula, String> {
    let tokens: Vec<BooleanExpressionToken<ALLAtom>> = tokenize_boolean_expression(&mut value.chars().peekable(), true, &|data, first| {
            ...
            if name == "AllAttractors" || name == "SomeAttractor" {
                while let Some(c) = data.peek() {
                    if c.is_whitespace() {
                        ...
                        if let Ok(tokens) = tokenize_boolean_expression(data, false, &|data, first| {
                            return if is_valid_in_name(first) {
                                // start of a variable name
                                let mut name = vec![first];
                                while let Some(c) = data.peek() {
                                ...
                                if name == "AllStates" || name == "SomeState" {
                                    while let Some(c) = data.peek() {
                                        if c.is_whitespace() {
                                            ...
                                            if let Ok(tokens) = tokenize_boolean_expression(data, false, &self::try_string_atom) {
                                                if let Ok(state_formula) = parse_boolean_expression(&tokens, &|atom: &String| {
                                                    Ok(StateAtom::IsSet(bn.graph().find_variable(atom.as_str()).unwrap()))
                                                }) {
                                                    return if name == "AllStates" {
                                                        Some(AttractorAtom::AllStates(*state_formula))
                                                    } else {
                                                        Some(AttractorAtom::SomeState(*state_formula))
                                                    }
                                                }
                                            } else {
                                                return None;
                                    ...
                                    return Some(AttractorAtom::IsClass(Behaviour::Stability));
                                } else if name == "oscillation" {
                                    return Some(AttractorAtom::IsClass(Behaviour::Oscillation));
                                } else if name == "disorder" {
                                    return Some(AttractorAtom::IsClass(Behaviour::Disorder));
                                }
                                None
                            } else {
                                None
                            ...
                            return if let Ok(attractor_formula) = parse_boolean_expression(&tokens, &|atom| {
                                Ok(atom.clone())
                            }) {
    ...
    let attractor_formula = parse_boolean_expression(&tokens, &|atom: &ALLAtom| {
        Ok(atom.clone())
    })?;
...
mod tests {
    ...
    fn tokenize_state_formula() {
        let bn = BooleanNetwork::try_from("\
            ...
            a -> c
        ").unwrap();
        let input = "SomeAttractor(AllStates(a & b | c) & SomeState(c <=> b)) && AllAttractors(disorder)";
